...
void very_quicksort(int *A, int p, int r, int limite, int *pindex, pthread_mutex_t *mutex, couple_t *couples,
...
{
    pargs_t *pargs = (pargs_t *)args;

    very_quicksort(pargs->A, pargs->p, pargs->r, pargs->limite, pargs->pindex, pargs->mutex, pargs->couples,
                   pargs->sem_free_slots, pargs->sem_data_items);
                    ...
                    sem_t *sem_free_slots, sem_t *sem_data_items)
{
    if (p < r)
    {
        ...
        else
        {
            ...
            pargs.pindex = pindex;
            pargs.mutex = mutex;
            pargs.couples = couples;
            pargs.sem_free_slots = sem_free_slots;
            pargs.sem_data_items = sem_data_items;

            // very_quicksort(A, p, q, limite, pindex, mutex, couples, sem_free_slots, sem_data_items); DIVENTA: 
            xpthread_create(&thread, NULL, pbody, &pargs, __LINE__, __FILE__);
           
            very_quicksort(A, q + 1, r, limite, pindex, mutex, couples, sem_free_slots, sem_data_items);
            xpthread_join(thread, NULL, __LINE__, __FILE__);
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    pthread_mutex_t cmutex, pmutex;
    xpthread_mutex_init(&cmutex, NULL, __LINE__, __FILE__);
    xpthread_mutex_init(&pmutex, NULL, __LINE__, __FILE__);
    ...
    very_quicksort(a, 0, n - 1, limite, &pindex, &pmutex, couples, &sem_free_slots, &sem_data_items);
