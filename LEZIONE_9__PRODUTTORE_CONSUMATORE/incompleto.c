...
int main(int argc, char *argv[])
{
  ...
  pthread_t t[p];
  dati a[p];
  sem_t sem_free_slots, sem_data_items, mutex;
  
  // inizializzazione semafori
  xsem_init(&sem_free_slots, 0, Buf_size);
  xsem_init(&sem_data_items, 0, 0);
  xsem_init(&mutex, 0, 1);
  ...
  for(i=0;i<p;i++) {
    ...
    a[i].mutex = &mutex;
    xpthread_create(&t[i], NULL, tbody, (void *) &a[i]);
  }
  ...
  while(true) {
    ...
    if(p==0) {
      ...
      xsem_wait(&sem_free_slots);
      buffer[pindex % Buf_size] = n;
      pindex += 1;
      xsem_post(&sem_data_items);
    }
  }
